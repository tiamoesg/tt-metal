...
//
// The low-rank latent c^Q = h W^{DQ} (eq 13) is computed by the *caller* and
// shared with the main attention queries (eq 18), so the indexer itself owns only
// W^{IUQ} and W^{w}. `index_scores` returns the differentiable scores I (used by
// the indexer's auxiliary training loss). `selection_mask` turns the scores into
// a 0/1 keep mask over compressed blocks (matching ttml's SDPA mask convention),
// enforcing the compressed-causal constraint that a query in block floor(t/m) may
// only attend to strictly earlier compressed blocks. Top-k selection is
// non-differentiable, so the mask carries no gradient.
struct LightningIndexerConfig {
    uint32_t dim{0};               // d, hidden size (input to W^{w})
    uint32_t index_query_dim{0};   // d_c, low-rank latent dim (input to W^{IUQ})
...
public:
    ...
    // hidden: [B, 1, S, d] (for the per-head weights W^{w}), latent: c^Q
    // [B, 1, S, d_c] (the shared low-rank query latent, for W^{IUQ}), index_keys
    // (K^IComp): [B, 1, G, c^I]. Returns differentiable index scores I: [B,1,S,G].
