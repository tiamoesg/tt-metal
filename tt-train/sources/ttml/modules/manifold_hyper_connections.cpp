...
autograd::TensorPtr ManifoldHyperConnections::operator()(const autograd::TensorPtr& expanded_residual) {
    ...
    return m_config.dynamic_parameterization ? compute_dynamic(expanded_residual) : compute_static(expanded_residual);
}

autograd::TensorPtr ManifoldHyperConnections::compute_static(const autograd::TensorPtr& x) {
...
autograd::TensorPtr ManifoldHyperConnections::compute_dynamic(const autograd::TensorPtr& x) {
    const auto x_shape = x->get_value().logical_shape().to_array_4D();
    const uint32_t batch = x_shape[0];
    const uint32_t seq = x_shape[2];
    const uint32_t dim = x_shape[3];
    ...
    auto f_out = (*m_inner_layer)(ax_bsd);                                                              // [B, 1, S, d]
