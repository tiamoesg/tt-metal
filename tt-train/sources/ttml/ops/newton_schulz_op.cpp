...
tt::tt_metal::Tensor newtonschulz_hybrid(const tt::tt_metal::Tensor& G, int steps, int stabilize_steps, float eps) {
    // Aggressive coefficients drive the singular values rapidly toward 1; the
    // stabilizing coefficients pin them at 1 in the final steps.
    constexpr float kAggr[3] = {3.4445f, -4.7750f, 2.0315f};
    constexpr float kStab[3] = {2.0f, -1.5f, 0.5f};
    ...
    const int aggressive_steps = (steps > stabilize_steps) ? (steps - stabilize_steps) : steps;
    for (int iter = 0; iter < steps; ++iter) {
        const float* k = (iter < aggressive_steps) ? kAggr : kStab;
        const float a = k[0];
        const float b = k[1];
        const float c = k[2];
